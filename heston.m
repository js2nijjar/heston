...
for i = 1:nsims
    for j=2:numel(tvec)
        ...
        svec(i,j) = max(svec(i,j),0);
        vvec(i,j) = max(vvec(i,j),0);
    end
end
...
payoffcall = max(mean(svec,2) - k, 0);
payoffput = max(k - mean(svec,2), 0);

T = days*dt;

callputprice = [mean(payoffcall)*exp(-c(1)*T),mean(payoffput)*exp(-c(1)*T)]
