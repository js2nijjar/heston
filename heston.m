...
for i = 1:nsims
    for j=2:numel(tvec)
        ...
        k1s = real(dt.*mu.*s + (dW1(j-1)-(plusminusone().*dt)).*v.*s);
        k1v = real(dt.*kappa.*(theta-v) + (dW2(j-1)-(plusminusone().*dt)).*sig.*s);
        k2s = real(dt.*mu.*(s+k1s) + (dW1(j-1)+(plusminusone().*dt)).*sqrt(v+k1v).*(s+k1s));
        k2v = real(dt.*kappa.*(theta-(v+k1v)) + (dW2(j-1)+(plusminusone().*dt)).*sig.*sqrt(v+k1v));
